...
# SQL 定義
sql_select_prompts = """
    SELECT image_id, generation_prompt
    FROM images
    WHERE prompt_embedding IS NULL AND generation_prompt IS NOT NULL
"""

sql_update_embedding = """
    UPDATE images
    SET prompt_embedding = :embedding
...
def process_prompts(select_cursor, update_cursor, batch_size=96):
    select_cursor.execute(sql_select_prompts)
    ...
    while True:
        ...
        image_ids, prompts = zip(*batch)
        # LOB型を文字列に変換
        prompts = [prompt.read() if hasattr(prompt, 'read') else prompt for prompt in prompts]
...
if __name__ == "__main__":
    try:
        ...
        total_processed, processing_time = process_prompts(select_cursor, update_cursor)
        end_time = time.time()
    ...
    print("画像生成プロンプトの embedding 生成と更新が完了しました。")
